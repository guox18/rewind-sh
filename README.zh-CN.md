...
- **输出流治理 (Stream Control)**：专为 Agent 阅读习惯设计的输出缓冲机制，结构化提供日志摘要（Head/Tail），并支持通过指令分页查看长日志。
...
每次启动会生成独立 `session id`，并写入独立的 state/log 目录，避免多个并发 shell 记录串台。  
...
`rewind-sh` 会在每条命令执行前，自动快照当前**受保护范围（roots）**内的文件与**环境变量**。
...
Agent 无法像人类一样使用 `less` 滚屏。`rewind-sh` 会将所有输出落盘，并提供分页读取 API。避免模型使用 `*` 等通配符导致的上下文爆炸。

**使用示例：**
```bash
# 在受控 shell 中执行常见命令
rewind-sh> find . -maxdepth 2 -type f

# 每条命令执行后会打印 record/snapshot/log 路径与输出摘要
# 如需查看完整日志，可直接查看上一步输出中的 log 文件
rewind-sh> cat /path/to/log.jsonl
```
...
- [ ] 在 Kubernetes Linux 容器中完成输出翻页核心功能的可用性验证
- [ ] 在常用 Linux 环境中测试
- [ ] 完成后台进程管理能力开发与测试
- [ ] 完成 macOS 兼容性适配与测试
